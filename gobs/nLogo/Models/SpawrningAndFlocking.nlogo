globals [ generationTime overallTime ]

patches-own [ attractor ]
...
to setup
  ...
  crt population
    ...
    [ set color yellow - 2 + random 7  ;; random shades look nice
      set size 2.5  ;; easier to see
      setxy min-pxcor min-pycor + random side-spacing
      createGroups
       ]
   ...
   crt population
    [ set color yellow - 2 + random 7  ;; random shades look nice
      set size 2.5  ;; easier to see
      setxy max-pxcor min-pycor + random side-spacing
      createGroups
       ]
  ;ask patch 0 35 [
   ; set attractor 1
  ;]
  ;repeat 100 [ diffuse attractor 0.5 ]
  reset-ticks
end
...
to go
  ;;ask turtles [ uphill pycor ]
  go-uphill
  ask turtles [ flock ]
  ;; the following line is used to make the turtles
  ;; animate more smoothly.
  repeat 1 [ ask turtles [ fd 0.2 flock ] display ]
  ;; flock-steps
  ;; for greater efficiency, at the expense of smooth
  ;; animation, substitute the following line instead:
   ;[ask turtles [ fd 1 ]]
    ifelse generationTime >= spawnTime
  [
    generateFish
    set generationTime random 25
  ]
  [
    set generationTime generationTime + 1
  ]
  ask turtles with [ xcor <= river-mouth + 10 and xcor >= river-mouth - 10 and ycor >= max-pycor - 5 ]
  [
    die
  ]
...
to generateFish
  ;;Generate fish at a range of random patches.
  ;;Sockeye runs appear to maximize around early July (around the 10th) and again around early August (1st - 10th). Spike greatest
  ;;in early July.
  ;;For each time program is created, set up two randomized time intervals around which runs will be increased.
  let runTiming2 runtiming * runSeperation
  ifelse (overallTime <= runTiming + 168 and overallTime >= runTiming - 168) or (overallTime <= runTiming2 + 168 and overallTime >= runTiming2 - 168)
  ;[
   ;  repeat 10 [
  ;ask one-of patches with [ pycor = min-pycor and pxcor >= -8 and pxcor <= 8 ]
  ;[
   ; sprout 1
    ;[
     ; hatch random 50
...
to go-uphill
  let flock-wiggle-half flock-wiggle / 2
   
  ask turtles [
    face patch random river-mouth max-pycor 
    set heading heading + random flock-wiggle - flock-wiggle-half
    fd 1]
end
