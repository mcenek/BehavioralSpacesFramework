...
globals [active-turtles ratio ratio2 wheel-number iterations energy-regen iterations2]
promotors-own[energy age new-heading nearest-attractor calclat calclng dst-nearest-attractor phistory history dist xc yc]
patches-own [penergy]
...
to report-position
  ask promotors [
    if ticks > 0 [
    set phistory (floor ((heading / 90) + 1))
    set phistory word phistory ","
    set history word phistory history]
  ]
end

to finish
  ask promotors [report-position]
end
...
to setup-patches
  ask patches [ set penergy 25 ]
end

to setup-promoters
  create-promotors 100 [setxy random-xcor random-ycor
    set energy 10 set color white]

end

to reproduce
  ask promotors [ifelse 10 <= promotor-reproduction-distance
  [ifelse energy >= 10
  [set energy 4
...
to kill-promoters
  ask promotors [ifelse energy-kills?
     [if energy = 0 [
         print-file
         die ]]
    []

    ;Swap out the 600 for however many ticks you want the actors to live for
    if age >= 600 [
      print-file
      die]]
end
