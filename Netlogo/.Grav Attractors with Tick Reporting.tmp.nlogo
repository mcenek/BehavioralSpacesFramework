...
globals [directory filename active-turtles ratio ratio2 wheel-number iterations energy-regen iterations2 vision]
promotors-own [fid new-heading nearest-attractor calclat calclng dst-nearest-attractor
  history x y px py pmov angle]
...
to report-position
  ;In order for reporting to work with world wrapping, model must:
  ;Have the point 0,0 in the left bottom corner
  ;Have no agents which are expected to report move more than 2*threshold each tick
  ;World must be larger than 2*threshold
  ask promotors [
    if (distancexy px py > threshold)
    [set x xcor
     set y ycor
     if ((px - x) > (2 * threshold))
     [set x (x + max-pxcor + 1)]
     if ((px - x) < (-2 * threshold))
     [set px (px + max-pxcor + 1)
       ]
     if ((py - y) > (2 * threshold))
     [set y (y + max-pycor + 1)
       ]
     if ((py - y) < (-2 * threshold))
     [set py (py + max-pycor + 1)
       ]
     set angle (atan (x - px) (y - py) )
     set pmov floor (angle / (360 / dims) )
     set pmov pmov + 1
     set pmov word pmov ","
     set pmov word pmov who
...
to files
  file-close-all
  set directory "C:\\Users\\Spencer\\Documents\\MATLAB\\Netlogo"
  set filename "20kticks"
  set filename word filename (vision * 2)
  ;set filename word filename ((iterations2 + 3) * 100)
  set filename word filename ".txt"
  print filename
  file-close-all
  carefully [file-delete filename][]
...
to update-labels
  ...
  ask promotors [ifelse dst-nearest-attractor <= vision [set color red] [ifelse show-inactives? [set color blue] [set color black]]]
...
to find-attractors
  ...
  ifelse dst-nearest-attractor <= vision [carefully [set new-heading towards nearest-attractor] []][]
